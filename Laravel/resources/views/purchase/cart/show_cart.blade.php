                                    ...
                                    @foreach($cart->products as $product)
                                        <tr>
                                            ...
                                            <td style="vertical-align: middle"><a class="product-name mt-4" href='{{route('product.show',$product->pivot->product_id)}}'>{{$product->product_name}}</a><br>
                                                Qty: <select value="{{$product->pivot->product_id}}" id="quantity" class="mt-2 mr-2 quantity_change">
                                                    <option  value=" ">{{$product->pivot->quantity}} {{$product->pivot->measure_unit}}</option>
                                                   @for($i=1;$i<=10;$i++)
                                                    <option  value="{{$i}}">{{$i}} {{$product->pivot->measure_unit}}</option>
                                                    @endfor
                                                </select>  |
                                                <button class="ml-2 btn btn-danger remove_product" value="{{$product->pivot->product_id}}" ><i class="fa fa-remove"></i></button>
                                            </td>

                                            <td style="vertical-align: middle" id="net_price_{{$product->pivot->product_id}}">Rs {{$product->pivot->net_price}}</td>
